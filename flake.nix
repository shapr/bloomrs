{
  ...
  outputs = {
    ...
    forAllSystems = function:
      nixpkgs.lib.genAttrs [
        ...
        bloomrs = craneLib.buildPackage (craneCommon
                                         // {
  ...
  in {
    checks = forAllSystems ({
      ...
      bloomrs-clippy = craneLib.cargoClippy (craneCommon
                                             // {
                                               inherit cargoArtifacts;
      ...
      # Check formatting
      bloomrs-fmt = craneLib.cargoFmt {
        inherit src;
      };

      # Audit dependencies
      bloomrs-audit = craneLib.cargoAudit {
    ...
    packages = forAllSystems ({ bloomrs, ... }: {
      inherit bloomrs;
      default = bloomrs;
    });
    ...
    apps = forAllSystems ({system, ...}: {
      bloomrs = {
        type = "app";
        program = "${self.packages.${system}.bloomrs}/bin/bloomrs";
      };
      default = self.apps.${system}.bloomrs;
    });
    ...
    devShells = forAllSystems ({
      ...
      default = pkgs.devshell.mkShell {
        # shellHook = ''
        # PKG_CONFIG_PATH=${pkgs.libudev-zero}
        # '';
        ...
        env = [
          ...
          }
          {
            ...
            value = pkgs.lib.concatStrings ["${pkgs.libudev-zero}" "/lib/pkgconfig"];
          }
        ];
      };
    });
